...
def load_images():
    ...
    for item_type in ['A', 'B', 'C']:
        for level in range(1, 8):  # Levels 1 through 7
            path = f"resources/slime_{item_type}_{level}.png"
...
def on_point_goal_reached():
    global point_goal, gen_level_to_reward
...
def handle_event(event):
    global dragging, dragged_item, dragged_pos, mouse_down_time, hide_dragged_item
    global running, total_score, show_help_text
...
def item_to_rendered_text(item, font):
    item_type, item_level = item
    if item[0] == 'G':  # Generator
        color = "darkgray"
        text = f"Gen {item_level}"
    else:  # Other items
        item_score = calculate_score(item)
        color = "red" if item_type == "A" else "blue"
        text = f"{item_type}{item_level} ({item_score})"
...
async def main():
    global running
    global dragging, hide_dragged_item
    while running:
